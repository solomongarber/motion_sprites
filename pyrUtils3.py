...
def putpt(im,pt,diam):
     if len(im.shape)==2:
         oim=np.dstack((im,im,im))
     else:
         oim=im.copy()
     red=np.reshape([0,0,np.max(im)],(1,1,3))
     lo_y=np.max([pt[0]-diam,0])
     lo_x=np.max([pt[1]-diam,0])
     hi_y=np.min([pt[0]+diam,im.shape[0]-1])
     hi_x=np.min([pt[1]+diam,im.shape[1]-1])
     oim[lo_y:hi_y,lo_x:hi_x,:]=red
     return oim
...
def decode(frame0, out_sig, xmos, ymos, gain, out=None, interp='push_resc_pull'):
...
def sim_xy(im,x_du,y_du,hz,s,ends,j,t,power,fps,interpolation='push',full_size=False,out=None):
    #print(interpolation)
    bgr=cv2.resize(im,(x_du.shape[1],x_du.shape[0]))
    ...
    ims('b',vh(mode_clp[:,:,1]))
    ims('c',vh(y_du[:,:,j]))
    #print(np.max(np.abs(mode_clp)))
    mode_clp/=np.max(np.abs(mode_clp))
    shp=mode_clp.shape
    bgr_clp=bgr[ends[0]:ends[1],ends[2]:ends[3],:]
    ims('bgr',bgr_clp)
    ims('i',vh(mode_clp[:,:,1]))
    x_inds=np.arange(im.shape[0]*im.shape[1]).reshape(im.shape[0],im.shape[1])%im.shape[1]
    y_inds=np.arange(im.shape[0]*im.shape[1]).reshape(im.shape[0],im.shape[1])//im.shape[1]
    inds=np.dstack((x_inds,y_inds))
    for i in range(t):
        state=np.exp(2j*np.pi*i*(hz/fps)-((s-.13)/27.4)*(i/fps))
        #print(state)
        #print(state.shape)
        ...
        if full_size:
            if interpolation=='push_pull':
                ...
                p=push_sparse(im,cv2.resize(disp+inds,(im.shape[1],im.shape[0])))
            if i%10==0:
                print (i)
        else:
            p=pull(bgr_clp,disp[:,:,::-1]+inds)
        ims('p',p)
        if out is not None:
            #print (p.shape)
            out.write(np.uint8(p))
...
def freq_html(checkpts_dir,chkpt,fixins,epoch,power,outframes):
    pref=checkpts_dir+'/'+chkpt+'/web/images/epoch'+epoch+'_'+fixins[0]+'_'
    prefA=checkpts_dir+'/'+chkpt+'/web/images/epoch'+epoch+'_real_'
    suf='_'+fixins[1]+'.png'
    sufA='_encoded.png'
    ...
    xmo=(xre+1j*xim).reshape(im.shape[0],-1,1)
    ymo=(yre+1j*yim).reshape(im.shape[0],-1,1)
    response=requests.get(pref+'C'+suf)
    resp=np.asarray(Image.open(BytesIO(response.content))) 
    sig=resp_to_sig(resp,outframes)
    decode(im,sig,xmo,ymo,power,interp='pull')
    #sim_xy(im,ymo,xmo,freq,.13,ends,0,t,power,fps)
    return (im,sig,ymo,xmo)


def resp_to_sig(resp,out_frames):
    response=np.mean(resp[:,:,0],0)
    response/=np.max(response)
    response=np.float32(response.reshape(-1,1))
    response=cv2.resize(response,(1,out_frames))
    sig= np.fft.ifft(response[:,0]*np.exp(2j*np.pi*np.random.rand(out_frames)))
    return np.dstack((sig,sig))
...
def val_from_html(checkpts_dir,inpt,sample):
    pref=checkpts_dir+'/input_'+inpt+'_'
    suf='_'+sample+'.png'
    response=requests.get(pref+'input.png')
    im=np.asarray(Image.open(BytesIO(response.content)))
    pref=pref+'random_sample_'
    response=requests.get(pref+'xr'+suf)
    xr=np.asarray(Image.open(BytesIO(response.content))) 
    xr=(np.float32(xr[:,:,0])-127)/128
    response=requests.get(pref+'xi'+suf)
    xi=np.asarray(Image.open(BytesIO(response.content))) 
    xi=(np.float32(xi[:,:,0])-127)/128
    response=requests.get(pref+'yr'+suf)
    yr=np.asarray(Image.open(BytesIO(response.content))) 
    yr=(np.float32(yr[:,:,0])-127)/128
    response=requests.get(pref+'yi'+suf)
    yi=np.asarray(Image.open(BytesIO(response.content)))        
    yi=(np.float32(yi[:,:,0])-127)/128
    response=requests.get(pref+'cc'+suf)
    curve=np.asarray(Image.open(BytesIO(response.content)))        
    curve=(np.float32(curve[:,:,0]))/255
    curve=np.sum(curve,0)
    response=requests.get(pref+'imag'+suf)
    return (im,yr+1j*yi,xr+1j*xi,curve)

def sim_val(inpt,sample,power=4,fps=4400,t=60,out=None):
    ckdir='http://gpgpu.cs-i.brandeis.edu/solomon/im2im/BicycleGAN/results/osc/val/images'
    (im,ymo,xmo,response)=val_from_html(ckdir,inpt,sample)
    ends=[0,im.shape[0],0,im.shape[1]]
    resp=np.zeros(800)
    resp[:256]=response
    (w,s)=quick_params(response,np.argmax(response))
    hz=(w*30)/800
    xmo=xmo.reshape(xmo.shape[0],xmo.shape[1],1)
    ymo=ymo.reshape(ymo.shape[0],ymo.shape[1],1)
    j=0
    s=.13
    sim_xy(im,ymo,xmo,hz,s,ends,j,t,power,fps,interpolation='push',out=out) 
    return (im,ymo,xmo,hz,s)
