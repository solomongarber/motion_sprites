...
whz=np.argsort(aft2[:400])[:num_modes];
w,s=pu.quick_params_multi(aft2[:400]/np.max(aft2),whz);
hz=w*fps/800;
sz=s*fps/800


xmos=freq_x[:,:,np.int32(w)].reshape(-1,freq_x.shape[1],1).copy()
ymos=freq_y[:,:,np.int32(w)].reshape(-1,freq_x.shape[1],1).copy()
...
for i in range(num_modes):
    ...
    lymo=np.log(ymo)
    lxmo=np.log(xmo)
    cv2.imwrite(bg_dir+'/y_'+str(i)+'_lamp.jpg',pu.ate(np.real(lymo)))
    cv2.imwrite(bg_dir+'/y_'+str(i)+'_phase.jpg',pu.ate(np.imag(lymo)))
    cv2.imwrite(bg_dir+'/x_'+str(i)+'_lamp.jpg',pu.ate(np.real(lxmo)))
    cv2.imwrite(bg_dir+'/x_'+str(i)+'_phase.jpg',pu.ate(np.imag(lxmo)))

    params[:,0,i]=(np.min(np.real(lymo)),np.max(np.real(lymo)))
    params[:,1,i]=(np.min(np.real(lxmo)),np.max(np.real(lxmo)))
...
in_sig=sig['out_sig']
fps=sig['fps']
newx=np.zeros(xmos.shape)
newy=np.zeros(ymos.shape)
newxp=newx.copy()
newyp=newx.copy()
for i in range(num_modes):
    newx[:,:,i]=cv2.imread(bg_dir+'/x_'+str(i)+'_lamp.jpg')[:,:,0]
    newx[:,:,i]=newx[:,:,i]*(params[1,1,i]-params[0,1,i])/255+params[0,1,i]
    newxp[:,:,i]=cv2.imread(bg_dir+'/x_'+str(i)+'_phase.jpg')[:,:,0]
    newxp[:,:,i]=2*(newxp[:,:,i]-127.5)*np.pi/255 
    newy[:,:,i]=cv2.imread(bg_dir+'/y_'+str(i)+'_lamp.jpg')[:,:,0]
    newy[:,:,i]=newy[:,:,i]*(params[1,0]-params[0,0])/255+params[0,0]
    newyp[:,:,i]=cv2.imread(bg_dir+'/y_'+str(i)+'_phase.jpg')[:,:,0]
    newyp[:,:,i]=2*(np.float64(newyp[:,:,i])-127.5)*np.pi/255
